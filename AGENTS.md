...
This pegs to **`Codex-haiku-4-5`** specifically. Switching model means revisiting both the weight constants and the `MODEL` const in `brain.ts` (e.g. Sonnet 4.6 input $3 / output $15 would shift the ratios). Keep these together — the weighting is meaningless if it doesn't match the model's pricing.
